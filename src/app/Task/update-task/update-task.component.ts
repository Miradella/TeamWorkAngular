...
export class UpdateTaskComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.task = new Project();
    this.id = this.route.snapshot.params['id'];

    this.ts.getTask(this.id)
      .subscribe(data => {
        console.log(data);
        this.ps.getProject(data.project.projectId)
          .subscribe(data1 => {
              console.log(data1);
              this.es.getEmployeesListbyTeam(data1.team.teamId)
                .subscribe(
                  datan => {
                    console.log(datan);
  ...
  save() {
    ...
    this.ts.updateTask(this.id, this.task).subscribe(data => {
      console.log(data);
      this.gotoList();
    }, error => console.log(error));
    this.task = new Task();
